...
import { gameFromApi } from './translator'

const gamesBackend = `${BACKEND}/games`

export default class GamesApi {
  static getGame (id: number): Promise<Game> {
    return safeFetch(`${gamesBackend}/${id}`).then(response => response.json()).then(json => gameFromApi(json))
  }

  static getGameByCode (gameCode: string): Promise<Game> {
    return safeFetch(`${gamesBackend}/${gameCode}`).then(response => response.json()).then(json => gameFromApi(json))
  }
